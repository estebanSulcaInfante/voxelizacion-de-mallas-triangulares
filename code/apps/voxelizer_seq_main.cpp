...
std::string resolveOutputPath(const std::string& userPath) {
    const fs::path outputRoot = "data/output/vtu";
...
void printUsage(const char* progName) {
    ...
    std::cout << "  - Toda salida VTU se guarda dentro de data/output/vtu" << std::endl;
    std::cout << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    Timer timer1;
    timer1.start();
    std::vector<uint32_t> actNodes1 = seq::determineActiveLevel1Nodes(mesh, d);
    timer1.stop();
    ...
    Timer timer2;
    timer2.start();
    SparseOctree octree = seq::buildSparseOctree(actNodes1, h);
    timer2.stop();
    ...
    Timer timer3;
    timer3.start();
    seq::voxelizeIntoOctree(mesh, octree, h);
    timer3.stop();
    ...
    uint64_t solidVoxels = seq::countSolidVoxels(octree);
