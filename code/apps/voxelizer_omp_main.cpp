...
int main(int argc, char* argv[]) {
    ...
    Timer timer1;
    timer1.start();
    std::vector<uint32_t> actNodes1 = par::determineActiveLevel1Nodes(mesh, d);
    timer1.stop();
    ...
    Timer timer2;
    timer2.start();
    SparseOctree octree = par::buildSparseOctree(actNodes1, h);
    timer2.stop();
    ...
    Timer timer3;
    timer3.start();
    par::voxelizeIntoOctree(mesh, octree, h);
    timer3.stop();
    ...
    uint64_t solidVoxels = par::countSolidVoxels(octree);
