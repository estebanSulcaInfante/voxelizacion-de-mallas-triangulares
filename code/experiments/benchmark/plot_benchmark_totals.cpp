...
double mapYLinear(double value,
                  double minVal,
                  double maxVal,
                  double top,
                  double plotHeight) {
...
void writeMetricSVG(const std::vector<PlotRow>& rows, const fs::path& outputPath, bool isSpeedup) {
    const int width = 1400;
    const int height = 900;
    const double left = 120.0;
    const double right = 320.0;
    ...
    std::set<int> threadSet;
    std::map<int, std::map<int, double>> groupedData;

    for (const PlotRow& row : rows) {
        threadSet.insert(row.threads);
        groupedData[row.resolution][row.threads] = row.totalMs;
    }

    std::vector<int> threadValues(threadSet.begin(), threadSet.end());
    if (threadValues.empty()) return;

    double minVal = 0.0;
    double maxVal = isSpeedup ? 16.0 : 100.0;
    ...
    out << "<style>\n"
        ...
        << ".tick { stroke: #111827; stroke-width: 1.5; }\n"
        << ".series { fill: none; stroke-width: 3; }\n"
        << ".ideal { stroke: #9ca3af; stroke-dasharray: 8 8; stroke-width: 2; }\n"
        << ".point { stroke: white; stroke-width: 1.5; }\n"
        << "</style>\n";

    std::string title = isSpeedup ? "Speedup por cantidad de hilos" : "Eficiencia por cantidad de hilos";
    out << "<text x=\"" << width / 2 << "\" y=\"50\" text-anchor=\"middle\" font-size=\"30\" font-weight=\"600\">"
        << xmlEscape(title) << "</text>\n";

    int tickStep = isSpeedup ? 2 : 10;
    for (int i = 0; i <= static_cast<int>(maxVal); i += tickStep) {
        double y = mapYLinear(i, minVal, maxVal, top, plotHeight);
        out << "<line class=\"grid\" x1=\"" << left << "\" y1=\"" << y
            << "\" x2=\"" << (left + plotWidth) << "\" y2=\"" << y << "\"/>\n";
        out << "<line class=\"tick\" x1=\"" << (left - 8) << "\" y1=\"" << y
            << "\" x2=\"" << left << "\" y2=\"" << y << "\"/>\n";
        out << "<text x=\"" << (left - 18) << "\" y=\"" << (y + 6)
            << "\" text-anchor=\"end\" font-size=\"20\">" << i << "</text>\n";
    }
    ...
    std::string yLabel = isSpeedup ? "Speedup" : "Eficiencia (%)";
    out << "<text x=\"40\" y=\"" << (top + plotHeight / 2.0)
        << "\" text-anchor=\"middle\" font-size=\"24\" transform=\"rotate(-90 40 "
        << (top + plotHeight / 2.0) << ")\">" << yLabel << "</text>\n";

    if (isSpeedup) {
        double x1 = mapX(1, threadValues, left, plotWidth);
        double y1 = mapYLinear(1.0, minVal, maxVal, top, plotHeight);
        double x16 = mapX(16, threadValues, left, plotWidth);
        double y16 = mapYLinear(16.0, minVal, maxVal, top, plotHeight);
        out << "<line class=\"ideal\" x1=\"" << x1 << "\" y1=\"" << y1
            << "\" x2=\"" << x16 << "\" y2=\"" << y16 << "\"/>\n";
    } else {
        double y100 = mapYLinear(100.0, minVal, maxVal, top, plotHeight);
        out << "<line class=\"ideal\" x1=\"" << left << "\" y1=\"" << y100
            << "\" x2=\"" << (left + plotWidth) << "\" y2=\"" << y100 << "\"/>\n";
    }

    size_t colorIndex = 0;
    for (const auto& [resolution, threadMap] : groupedData) {
        const std::string& color = colors[colorIndex % colors.size()];
        ++colorIndex;
        
        double t1 = 0.0;
        if (threadMap.count(1)) t1 = threadMap.at(1);

        std::vector<std::pair<int, double>> points;
        for (const auto& [threads, totalMs] : threadMap) {
            double speedup = (t1 > 0) ? (t1 / totalMs) : 0.0;
            double efficiency = (threads > 0 && speedup > 0) ? (speedup / threads) * 100.0 : 0.0;
            double val = isSpeedup ? speedup : efficiency;
            points.push_back({threads, val});
        }
        
        std::sort(points.begin(), points.end());

        out << "<polyline class=\"series\" stroke=\"" << color << "\" points=\"";
        for (const auto& [threads, val] : points) {
            double x = mapX(threads, threadValues, left, plotWidth);
            double y = mapYLinear(val, minVal, maxVal, top, plotHeight);
            out << formatDouble(x) << "," << formatDouble(y) << " ";
        }
        out << "\"/>\n";
    ...
    const double legendLine = 34.0;
    out << "<rect x=\"" << (legendX - 20.0) << "\" y=\"" << (legendY - 30.0)
        << "\" width=\"240\" height=\"" << (groupedData.size() * legendLine + 40.0)
        << "\" rx=\"12\" fill=\"white\" stroke=\"#d1d5db\"/>\n";
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::string speedupStr = outStr;
        std::string efficiencyStr = outStr;
        if (outStr.size() > 4 && outStr.substr(outStr.size() - 4) == ".svg") {
            speedupStr = outStr.substr(0, outStr.size() - 4) + "_speedup.svg";
            efficiencyStr = outStr.substr(0, outStr.size() - 4) + "_efficiency.svg";
        } else {
            speedupStr += "_speedup.svg";
            efficiencyStr += "_efficiency.svg";
        }
        ...
        writeMetricSVG(rows, speedupStr, true);
        writeMetricSVG(rows, efficiencyStr, false);
        
        printMetrics(rows);

        std::cout << "Graficos generados:\n"
                  << "  - " << outputPath << "\n"
                  << "  - " << speedupStr << "\n"
                  << "  - " << efficiencyStr << std::endl;
        return 0;
